					...
					(response) ->
						for article in response.data.articles
							if $scope.blog.articles.length == 0
								$scope.blog.articles.push( { article: article, body_display: article.body.substr(0, 500)+"..." } )
							else
								$scope.blog.articles.push( { article: article, body_display: article.body.substr(0, 250)+"..." } )
